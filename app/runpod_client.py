...
class AsyncRunPodClient:
    ...
    async def _make_request(self, endpoint: str, payload: Dict) -> Dict:
        ...
        try:
            logger.debug(f"RunPod request to {url}: {json.dumps(payload)[:500]}...")
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                async with session.post(url, headers=self.headers, json=payload) as response:
                    if response.status != 200:
                        error_text = await response.text()
                        error_msg = f"RunPod API error: {response.status} - {error_text}"
                        logger.error(error_msg)
                        raise LLMError(error_msg)

                    return await response.json()
        except aiohttp.ClientError as e:
            error_msg = f"RunPod API request failed: {str(e)}, URL: {url}, Payload: {json.dumps(payload)[:100]}..."
            logger.error(error_msg)
            raise LLMError(error_msg)
        except asyncio.TimeoutError:
            error_msg = f"RunPod API request timed out after {self.timeout.total} seconds"
            logger.error(error_msg)
            raise LLMError(error_msg)
